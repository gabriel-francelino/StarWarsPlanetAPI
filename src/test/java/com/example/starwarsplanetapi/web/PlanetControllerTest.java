...
public class PlanetControllerTest {
    ...
    public void getPlanet_ByUnexistingName_ReturnsNotFound() throws Exception{
        ...
        mockMvc.perform(get("/planets/name/{name}", "Unexisting name"))
                .andExpect(status().isNotFound());
    ...
    public void removePlanet_WithUnexistingId_ReturnsNotFound() throws Exception {
        doThrow(new EmptyResultDataAccessException(1)).when(planetService).remove(-99L);

        mockMvc.perform(delete("/planets/{id}", -99L))
                .andExpect(status().isNotFound());
    }
